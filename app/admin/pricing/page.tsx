  ...
  return <main style={{minHeight:"100vh",background:"#f6f7f9",color:"#17233b",padding:"18px 24px 40px"}}>
    <div style={{maxWidth:1400,margin:"auto"}}>
      <header style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:20,marginBottom:12}}>
        ...
        <Link href="/admin" style={{padding:"9px 12px",border:"1px solid #d0d5dd",borderRadius:8,textDecoration:"none",color:"#344054",background:"white"}}>← Backoffice</Link>
      </header>
      ...
      <section style={{marginTop:12,background:"white",border:"1px solid #e4e7ec",borderRadius:12,padding:15}}>
        <h2 style={{margin:"0 0 8px",fontSize:17}}>Reglas activas</h2>
      ...
      <section style={{marginTop:12,background:"white",border:"1px solid #e4e7ec",borderRadius:12,padding:15}}>
        <h2 style={{margin:"0 0 8px",fontSize:17}}>Simulador de precio</h2>
        ...
        </div><button disabled={busy==="simulate"} style={{marginTop:10,padding:"9px 13px",border:0,borderRadius:8,background:"#ff6b4a",color:"white",fontWeight:750}}>{busy==="simulate"?"Calculando…":"Simular"}</button></form>
        {simulation?<div style={{marginTop:12,display:"grid",gridTemplateColumns:"280px 1fr",gap:14}}><div style={{background:"#f8fafc",padding:12,borderRadius:9}}><small>Precio base</small><strong style={{display:"block",fontSize:20}}>{simulation.currency} {simulation.base_amount.toFixed(2)}</strong><small>Ajuste total</small><strong style={{display:"block"}}>{simulation.currency} {simulation.delta_amount.toFixed(2)}</strong><small>Precio final</small><strong style={{display:"block",fontSize:25,color:"#10223f"}}>{simulation.currency} {simulation.total_amount.toFixed(2)}</strong></div><div>{simulation.applied.length?simulation.applied.map(a=><div key={a.id} style={{display:"flex",justifyContent:"space-between",padding:"7px 0",borderBottom:"1px solid #eef0f3"}}><span>{a.program_name?`${a.program_name} · `:""}{a.name}</span><b>{a.amount>=0?"+":""}{simulation.currency} {a.amount.toFixed(2)}</b></div>):<p style={{color:"#667085"}}>No aplicó ninguna regla.</p>}</div></div>:null}
      </section>
    </div>
  </main>;
}
