...
export async function PATCH(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const upstream = await fetch(`${provider.apiUrl}/api/admin/catalog/${encodeURIComponent(id)}`, {
    method: "PATCH",
  ...
  if (!upstream.ok) return noStoreJson({ message: backendMessage(payload, "No pudimos actualizar el producto.") }, upstream.status || 502);
