...
import { accessConfiguration, backendMessage, demoModeEnabled, noStoreJson, parseJson, providerHeaders, RUMBO_SESSION_COOKIE } from "../../../../../lib/rumbo-access";
...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  if (!token && !demoModeEnabled()) return noStoreJson({ message: "No hay una sesión administrativa activa." }, 401);
...
export async function PATCH(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const upstream = await fetch(`${provider.apiUrl}/api/admin/reservations/${encodeURIComponent(id)}`, {
    method: "PATCH", headers: providerHeaders(provider, { token, json: true }), body: JSON.stringify(body), cache: "no-store",
  });
  const payload = await parseJson(upstream);
