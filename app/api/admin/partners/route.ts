...
export async function POST(request: NextRequest) {
  ...
  try {
    const upstream = await fetch(`${provider.apiUrl}/api/admin/partners`, { method:"POST", headers:providerHeaders(provider,{ token, json:true, demoRole:"wholesaler_admin" }), body:JSON.stringify(body), cache:"no-store" });
    const payload = await parseJson(upstream);
    if (!upstream.ok) return noStoreJson({ message: backendMessage(payload, "No pudimos crear el Partner.") }, upstream.status || 502);
