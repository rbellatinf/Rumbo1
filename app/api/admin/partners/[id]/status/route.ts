...
export async function PATCH(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const upstream = await fetch(`${provider.apiUrl}/api/admin/partners/${encodeURIComponent(id)}/status`, {
    method: "PATCH",
  ...
  if (!upstream.ok) return noStoreJson({ message: backendMessage(payload, "No pudimos actualizar el Partner.") }, upstream.status || 502);
