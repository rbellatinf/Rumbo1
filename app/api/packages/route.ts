...
function chooseDeparture(product: NativeProduct, input: { originIata: string; departureDate: string; returnDate: string; adults: number }) {
  return (Array.isArray(product.departures) ? product.departures : [])
    .filter((departure) => {
      ...
      if (departure.sale_open === false) return false;
      if (departure.sale_deadline && Date.parse(departure.sale_deadline) < Date.now()) return false;
      ...
      const min = Number(departure.min_passengers_per_booking || 1);
      const max = Number(departure.max_passengers_per_booking || 9);
      if (input.adults < min || input.adults > max) return false;
...
function nativePackage(product: NativeProduct, departure: NativeDeparture): TravelPackage {
  ...
  const confirmationLabel = departure.confirmation_label || (departure.confirmation_mode === "minimum_required" ? "Sujeta a mínimo de pasajeros" : "Salida confirmada");
  ...
  return {
    ...
    previousPrice: "",
    tag: lowStock ? `Últimos ${remaining} cupos` : confirmationLabel,
    ...
    currency,
    bookable: Boolean(departure.id && amount > 0 && departure.sale_open !== false && (remaining == null || remaining > 0)),
...
export async function GET(request: NextRequest) {
  ...
  if (!IATA_CODE.test(originIata) || !IATA_CODE.test(destinationIata) || !ISO_DATE.test(departureDate) || !ISO_DATE.test(returnDate) || !Number.isInteger(adults) || adults < 1 || adults > 18 || Date.parse(returnDate) <= Date.parse(departureDate)) {
    return NextResponse.json({ mode: "demo", provider: "Rumbo", packages: [], message: "Selecciona aeropuertos válidos, fechas consecutivas y entre 1 y 18 viajeros." }, { status: 400 });
  }
  ...
  if (nativePackages && nativePackages.length > 0) {
    return NextResponse.json({ mode: "live", provider: "Rumbo", packages: nativePackages, message: `Encontramos ${nativePackages.length} opción${nativePackages.length === 1 ? "" : "es"} propia${nativePackages.length === 1 ? "" : "s"} de Rumbo para ${destinationName}, considerando hasta ±${FLEX_DAYS} días.` }, { headers: { "Cache-Control": "private, max-age=15" } });
  }
  ...
  return NextResponse.json({ ...result, message: nativePackages !== null ? `No encontramos una salida propia de Rumbo abierta para venta, compatible con ${adults} pasajero(s) y saliendo desde ${originIata} dentro de ±${FLEX_DAYS} días. ${result.message}` : result.message }, { headers: { "Cache-Control": "private, max-age=30" } });
}
