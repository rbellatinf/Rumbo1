...
export async function POST(request: NextRequest) {
  ...
  try {
    const upstream = await fetch(`${provider.apiUrl}/api/access/reset-password`, {
      method: "POST",
    ...
    if (!upstream.ok) return noStoreJson({ message: backendMessage(payload, "No pudimos actualizar la contraseña.") }, upstream.status || 502);
