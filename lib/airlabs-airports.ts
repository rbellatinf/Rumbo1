...
type AirLabsResponse = AirLabsSuggestions & {
  response?: AirLabsSuggestions;
  error?: {
    code?: string;
    message?: string;
  };
...
const demoAirports: AirportOption[] = [
  ...
  },
  {
    ...
    countryName: "Panamá",
    subType: "AIRPORT",
    label: "Ciudad de Panamá (PTY) · Tocumen, Panamá",
  },
  {
    id: "CUN",
    iataCode: "CUN",
    ...
    .filter((airport) => {
      ...
      return fields.some(
        (field) =>
          ...
          field.split(/[^a-z0-9]+/).some((word) => word.startsWith(query)),
      );
    })
    .slice(0, 8);
}

function regionName(countryCode?: string, providedName?: string) {
...
function mapAirport(
  ...
  const name = airport.name?.trim() || iataCode;
  const cityName = airport.city?.trim() || name;
  const countryCode = airport.country_code?.trim().toUpperCase();
  const countryName = regionName(
    countryCode,
    countryCode ? countries.get(countryCode) : undefined,
  );

  return {
    id: `AIRPORT-${airport.icao_code?.trim() || iataCode}`,
    iataCode,
    name,
    cityName,
    countryName,
...
function mapCity(
  ...
  const iataCode = city.city_code?.trim().toUpperCase();
  if (!iataCode || iataCode.length !== 3) return null;

  const cityName = city.name?.trim() || iataCode;
...
function uniqueOptions(options: AirportOption[]) {
  const seen = new Set<string>();
  return options.filter((option) => {
    const key = `${option.subType}-${option.iataCode}`;
...
export async function searchAirports(
  ...
  const baseUrl = (
    process.env.AIRLABS_API_BASE_URL || "https://airlabs.co/api/v9"
  ).replace(/\/$/, "");

  if (!apiKey || keyword.length < 3) {
    return {
      mode: "demo",
      provider: "AirLabs",
      airports: demoSearch(keyword),
      message: apiKey
        ? "Escribe al menos tres letras para consultar AirLabs."
        : "Catálogo local de respaldo. Añade la clave de AirLabs para consultar aeropuertos mundiales.",
    };
  }
  ...
  try {
    const query = new URLSearchParams({
      ...
      lang: "es",
      api_key: apiKey,
      _fields: "name,iata_code,icao_code,city,city_code,country_code",
    });
    const response = await fetch(`${baseUrl}/suggest?${query.toString()}`, {
    ...
    if (!response.ok) {
      throw new Error(`AirLabs suggestions returned ${response.status}`);
    }

    const payload = (await response.json()) as AirLabsResponse;
    if (payload.error) {
      throw new Error(payload.error.message || payload.error.code || "AirLabs error");
    }
    ...
    const countries = new Map(
      (suggestions.countries ?? [])
        ...
        .map((country) => [country.code!.trim().toUpperCase(), country.name!.trim()]),
    );
    const airports = uniqueOptions([
      ...(suggestions.cities ?? [])
        .map((city) => mapCity(city, countries))
        .filter((option): option is AirportOption => option !== null),
      ...(suggestions.airports ?? [])
        .map((airport) => mapAirport(airport, countries))
        .filter((option): option is AirportOption => option !== null),
    ]).slice(0, 10);
    ...
    return {
      ...
      provider: "AirLabs",
      airports,
      message: "Aeropuertos consultados en AirLabs.",
    };
  } catch {
    return {
      mode: "demo",
      ...
      airports: demoSearch(keyword),
      message:
        "AirLabs no respondió; se muestran aeropuertos del catálogo local de respaldo.",
    };
  } finally {
