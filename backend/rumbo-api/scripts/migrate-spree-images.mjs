...
const manifestPath = path.resolve(here, "../../../infra/cloudflare/legacy-catalog-images.json");
...
async function loadManifest() {
  try {
    ...
    console.warn("Spree image migration: no se pudo leer el manifiesto legacy:", error.message);
    return { products: [] };
  }
}
...
function looksLikeImageUrl(url, pathText = "") {
  if (/\.(jpe?g|png|webp|gif|avif|svg)(?:[?#]|$)/i.test(url)) return true;
  if (/(image|photo|picture|thumbnail|hero|cover|asset|attachment|cdn)/i.test(pathText)) return true;
  if (/(r2\.dev|cloudflare|cloudflarestorage|imagedelivery\.net)/i.test(url)) return true;
  return false;
}

function urlScore(url, pathText = "") {
  let score = 0;
  ...
  if (/(image_url|image|photo|picture|thumbnail|hero|cover)/i.test(pathText)) score += 40;
  if (/(url|asset|attachment|blob)/i.test(pathText)) score += 20;
  return score;
}

function collectDirectUrls(value, pathParts = [], output = []) {
  ...
  if (Array.isArray(value)) {
    value.forEach((item, index) => collectDirectUrls(item, [...pathParts, String(index)], output));
    return output;
  }
  if (typeof value === "object") {
    for (const [key, nested] of Object.entries(value)) collectDirectUrls(nested, [...pathParts, key], output);
  }
  return output;
}

function objectUrl(baseUrl, key) {
  return `${clean(baseUrl).replace(/\/$/, "")}/${clean(key).replace(/^\/+/, "").split("/").map(encodeURIComponent).join("/")}`;
}

function blobDerivedUrls(snapshot, publicBase) {
  ...
  return blobs.flatMap((blob, index) => {
    ...
    return [{
      url: objectUrl(publicBase, key),
      path: `active_storage_blobs.${index}.key`,
      score: 190,
      blob,
...
function manifestCandidates(link, manifest, publicBase) {
  if (!publicBase) return [];
  const entries = Array.isArray(manifest?.products) ? manifest.products : [];
  const productName = norm(link.name);
  return entries
    .filter((entry) => {
      if (clean(entry.source_system) && clean(entry.source_system) !== "spree") return false;
      if (clean(entry.source_id) === String(link.source_id)) return true;
      const match = norm(entry.match);
      return Boolean(match && productName.includes(match));
    })
    .filter((entry) => clean(entry.object_key))
    .map((entry) => ({
      url: objectUrl(publicBase, entry.object_key),
      path: "legacy_catalog_image_manifest",
      score: 250,
      source: "legacy_manifest",
      manifestEntry: entry,
      blob: {
        key: clean(entry.object_key),
...
function uniqueCandidates(snapshot, extraCandidates, publicBase) {
  const candidates = [
    ...extraCandidates,
    ...blobDerivedUrls(snapshot, publicBase),
    ...collectDirectUrls(snapshot),
  ];
  const byUrl = new Map();
...
async function main() {
  ...
  const publicBase = clean(process.env.CLOUDFLARE_R2_PUBLIC_BASE_URL || manifest.public_base_url).replace(/\/$/, "");
  if (!publicBase) console.warn("Spree image migration: no hay base pública R2 disponible.");
  ...
  for (const link of links) {
    ...
    const mapped = manifestCandidates(link, manifest, publicBase);
    const candidates = uniqueCandidates(snapshot, mapped, publicBase);
    ...
    for (const candidate of candidates) {
      ...
      const metadata = {
        ...
        duplicate_legacy_object_key: clean(candidate.manifestEntry?.duplicate_legacy_object_key) || null,
    ...
    if (finalCount > 0) productsWithImage += 1;
      ...
      warnings.push({
        ...
        product: link.name,
        warning: "No se encontró una imagen legacy utilizable.",
  ...
  console.log(`Spree image public base: ${publicBase || "NO_CONFIGURADA"}`);
