...
const sampleCatalogFiles=new Set(["101_rumbo_test_catalog_seed.sql","102_rumbo_world_test_catalog_seed.sql"]);
const loadSampleCatalog=/^(1|true|yes)$/i.test(process.env.RUMBO_LOAD_SAMPLE_CATALOG||"");
const migrations=(await fs.readdir(initDir))
  .filter(file=>/^\d+_.+\.sql$/.test(file))
  .filter(file=>Number(file.match(/^(\d+)_/)?.[1]||0)>=70)
  .filter(file=>!productionExcluded.test(file)||(loadSampleCatalog&&sampleCatalogFiles.has(file)))
...
try{
  ...
  console.log(`Rumbo DB prepare complete: ${migrations.length} migrations; sample_catalog=${loadSampleCatalog?"enabled":"disabled"} (${migrations.at(-1)||"none"}).`);
