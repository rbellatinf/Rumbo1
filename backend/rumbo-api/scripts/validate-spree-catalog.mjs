...
try {
  ...
  if(!exists){ console.log('Spree migration validation: no existe spree_products; catálogo legacy ya no está presente.'); process.exitCode=0; }
    ...
    for(const target of ['panama','miami']){
      ...
      const native=await pool.query(`SELECT p.id,p.name,p.slug,p.country,p.country_code,p.city,p.destination_iata,p.status,g.region_name,g.subregion_name,(SELECT count(*)::int FROM rumbo_catalog_departures d WHERE d.product_id=p.id) AS departures,(SELECT count(*)::int FROM rumbo_catalog_images i WHERE i.product_id=p.id) AS images,(SELECT i.url FROM rumbo_catalog_images i WHERE i.product_id=p.id ORDER BY i.is_primary DESC,i.sort_order,i.created_at LIMIT 1) AS image_url FROM rumbo_catalog_products p LEFT JOIN rumbo_catalog_product_geography g ON g.product_id=p.id WHERE rumbo_geo_normalize(COALESCE(p.name,'')||' '||COALESCE(p.city,'')||' '||COALESCE(p.country,'')) LIKE $1 ORDER BY p.created_at DESC`,[`%${target}%`]);
      ...
      for(const row of native.rows) console.log(`  Rumbo ${row.name} | ${row.country_code||'--'} | ${row.region_name||'sin región'} / ${row.subregion_name||'sin subregión'} | salidas=${row.departures} | imágenes=${row.images} | ${row.image_url||'sin foto'}`);
    }
    ...
    for(const target of ['panama','miami']) {
      if(targets[target].source_found>0&&targets[target].native_found===0){ console.error(`${target} existe en Spree y no fue migrado.`); process.exitCode=3; }
      if(targets[target].source_found>0&&targets[target].native_found>0&&!targets[target].native.some(row=>Number(row.images)>0)){
        console.error(`${target} fue migrado pero sigue sin imagen nativa en rumbo_catalog_images.`);
        process.exitCode=4;
      }
    }
  }
