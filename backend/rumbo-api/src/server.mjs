...
function requireAdmin(req, res, next) {
  if (req.rumboSession?.role !== "wholesaler_admin") {
    return res.status(403).json({ error: { message: "Este recurso requiere permisos de administración de Rumbo." } });
  }
  next();
}
...
async function accountPayload(accountId) {
  ...
  if (account.role === "wholesaler_admin") {
    return { account, profile: { type: "admin" }, redirect_to: "/admin" };
  }
...
function referralStem(firstName, lastName) {
  const stem = `${firstName}-${lastName}`
    .normalize("NFD").replace(/[\u0300-\u036f]/g, "")
    .toUpperCase().replace(/[^A-Z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 22);
  return stem || "PARTNER";
}
  ...
  await pool.query(
    `INSERT INTO rumbo_accounts (email,password_hash,role,status,email_verified_at)
     ...
     ON CONFLICT ((lower(email))) DO UPDATE
       SET role='wholesaler_admin', status='active', updated_at=now()`,
    [email, passwordHash],
  );
}
...
app.post("/api/access/register", async (req, res) => {
  ...
  try {
    ...
    if (accessType === "partner") {
      ...
      const documentNumber = clean(req.body.document_number).toUpperCase();
      ...
      let referralCode;
      do {
        referralCode = `RUMBO-${referralStem(firstName,lastName)}-${crypto.randomBytes(2).toString("hex").toUpperCase()}`;
      ...
      const { rows: associates } = await client.query(
        `INSERT INTO rumbo_associates (spree_customer_id,referral_code,membership_status,direct_commission_rate)
         VALUES ($1,$2,'pending',0.06) RETURNING id`,
        [`rumbo-account:${accountId}`, referralCode],
      );
      await client.query(
      ...
      if (sponsor) await client.query(
        ...
        [sponsor.account_id, accountId, sponsor.referral_code],
      );
    } else {
      const legalName = clean(req.body.legal_name);
      const tradeName = clean(req.body.trade_name);
      const taxId = clean(req.body.tax_id).replace(/\D/g, "");
      const representative = clean(req.body.representative);
      const phone = clean(req.body.phone);
      if (!legalName || !tradeName || !taxId || !representative) throw new Error("RETAILER_FIELDS");
      const { rows: retailers } = await client.query(
        `INSERT INTO rumbo_retailers (legal_name,trade_name,tax_id,phone,contact_email,status)
      ...
      await client.query(
        ...
        [retailers[0].id, accountId, parts[0], parts[1] || "—", phone || null],
      );
    }
    ...
    console.error(error);
    res.status(500).json({ error: { message: "No pudimos crear la cuenta." } });
...
app.patch("/api/commission-settings", requireSession, requireAdmin, async (req, res) => {
  ...
  const { rows } = await pool.query(
    `INSERT INTO rumbo_global_commission_settings(id,partner_rate,sponsor_rate,retailer_rate)
     VALUES(1,$1,$2,$3)
     ON CONFLICT(id) DO UPDATE SET partner_rate=EXCLUDED.partner_rate,sponsor_rate=EXCLUDED.sponsor_rate,retailer_rate=EXCLUDED.retailer_rate,updated_at=now()
     RETURNING partner_rate::float8,sponsor_rate::float8,retailer_rate::float8,updated_at`,
    values,
  );
  await audit(req.rumboSession.email, "commission_settings.updated", "commission_settings", 1, rows[0]);
  res.json(rows[0]);
});
...
app.get("/api/partner/dashboard", requireSession, async (req, res) => {
  ...
  const { rows: profiles } = await pool.query(
    `SELECT p.account_id,p.associate_id,p.first_name,p.last_name,p.referral_code,
            COALESCE(a.membership_status,account.status) AS membership_status,
            COALESCE(g.partner_rate,0.06)::float8 AS commission_rate,
            COALESCE(g.sponsor_rate,0)::float8 AS sponsor_rate
       FROM rumbo_partner_profiles p
       JOIN rumbo_accounts account ON account.id=p.account_id
       LEFT JOIN rumbo_associates a ON a.id=p.associate_id
  ...
  const [reservationsResult,salesResult,commissionsResult,networkResult] = await Promise.all([
    pool.query(
      `SELECT reference,product_name,contact_name AS customer,status,
              COALESCE((SELECT status FROM rumbo_booking_payments bp WHERE bp.booking_request_id=b.id LIMIT 1),'pending') AS payment_status,
              total_amount::float8,currency,departure_date,return_date,created_at
         FROM rumbo_booking_requests b WHERE referral_code=$1 ORDER BY created_at DESC LIMIT 50`,
      [profile.referral_code],
    ),
    pool.query(
      `SELECT sa.spree_order_id AS reference,b.contact_name AS customer,b.product_name,
              sa.gross_amount::float8,sa.currency,sa.payment_status,c.commission_amount::float8,
              c.status AS commission_status,sa.confirmed_at AS attributed_at
         FROM rumbo_sale_attributions sa
         LEFT JOIN rumbo_booking_requests b ON b.id=sa.booking_request_id
         LEFT JOIN rumbo_commissions c ON c.sale_attribution_id=sa.id AND c.beneficiary_type='partner' AND c.beneficiary_id=$1
    ...
    pool.query(
      `SELECT currency,
        ...
        WHERE beneficiary_id=$1 AND beneficiary_type IN('partner','sponsor') GROUP BY currency`,
      [accountId],
    ),
    pool.query(
      `SELECT p.account_id,concat_ws(' ',p.first_name,p.last_name) AS name,p.referral_code,a.status,p.created_at AS joined_at
        ...
        WHERE p.sponsor_partner_id=$1 ORDER BY p.created_at DESC`,
      [accountId],
    ),
  ]);
  ...
  for (const row of commissionsResult.rows) {
    accumulated[row.currency] = Number(row.accumulated || 0);
    pending[row.currency] = Number(row.pending || 0);
  }
  res.json({
    profile,
    metrics: {
      reservations: reservationsResult.rowCount,
      confirmed_sales: salesResult.rows.filter((sale) => ["confirmed","paid"].includes(sale.payment_status)).length,
...
app.get("/api/admin/overview", requireSession, requireAdmin, async (_req, res) => {
  const [counts, partners, retailers, commissions, settings, auditRows] = await Promise.all([
    pool.query(`SELECT
      (SELECT count(*) FROM rumbo_partner_profiles)::int AS partners,
      (SELECT count(*) FROM rumbo_accounts WHERE role='partner' AND status='pending')::int AS partners_pending,
      (SELECT count(*) FROM rumbo_retailers)::int AS retailers,
      (SELECT count(*) FROM rumbo_retailers WHERE status='pending')::int AS retailers_pending,
      (SELECT count(*) FROM rumbo_commissions)::int AS commissions,
      (SELECT count(*) FROM rumbo_commissions WHERE status='pending')::int AS commissions_pending`),
    pool.query(`SELECT p.account_id,p.first_name,p.last_name,p.document_type,p.document_number,p.phone,p.referral_code,a.email,a.status,p.created_at,
                       (SELECT count(*) FROM rumbo_referral_relationships rr WHERE rr.sponsor_partner_id=p.account_id AND rr.status='active')::int AS direct_referrals
                  FROM rumbo_partner_profiles p JOIN rumbo_accounts a ON a.id=p.account_id
                 ORDER BY p.created_at DESC LIMIT 100`),
    pool.query(`SELECT r.id,r.legal_name,r.trade_name,r.tax_id,r.contact_email,r.phone,r.status,r.created_at,
                       (SELECT count(*) FROM rumbo_retailer_members m WHERE m.retailer_id=r.id)::int AS member_count
                  FROM rumbo_retailers r ORDER BY r.created_at DESC LIMIT 100`),
    pool.query(`SELECT c.id,c.beneficiary_type,c.beneficiary_id,c.currency,c.base_amount::float8,c.rate::float8,c.commission_amount::float8,c.status,c.created_at,
                       sa.spree_order_id AS reference
                  FROM rumbo_commissions c JOIN rumbo_sale_attributions sa ON sa.id=c.sale_attribution_id
  ...
  res.json({
    ...
    commission_settings: settings.rows[0] || { partner_rate: 0.06, sponsor_rate: 0, retailer_rate: 0 },
    audit: auditRows.rows,
  });
});

app.patch("/api/admin/partners/:id/status", requireSession, requireAdmin, async (req, res) => {
  const status = clean(req.body.status);
  if (!["pending","active","blocked","disabled"].includes(status)) return res.status(422).json({ error: { message: "Estado de Partner inválido." } });
  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    const { rows } = await client.query(`UPDATE rumbo_accounts SET status=$2,updated_at=now() WHERE id=$1 AND role='partner' RETURNING id,email,status`, [req.params.id,status]);
    if (!rows[0]) { await client.query("ROLLBACK"); return res.status(404).json({ error: { message: "Partner no encontrado." } }); }
    await client.query(`UPDATE rumbo_associates SET membership_status=CASE WHEN $2='active' THEN 'active' WHEN $2='pending' THEN 'pending' ELSE 'suspended' END,updated_at=now() WHERE id=(SELECT associate_id FROM rumbo_partner_profiles WHERE account_id=$1)`, [req.params.id,status]);
    await client.query("COMMIT");
    await audit(req.rumboSession.email, "partner.status_changed", "partner", req.params.id, { status });
    res.json(rows[0]);
  } catch (error) { await client.query("ROLLBACK").catch(()=>{}); console.error(error); res.status(500).json({ error: { message: "No pudimos actualizar el Partner." } }); }
  finally { client.release(); }
});

app.patch("/api/admin/retailers/:id/status", requireSession, requireAdmin, async (req, res) => {
  const status = clean(req.body.status);
  if (!["pending","active","suspended","rejected"].includes(status)) return res.status(422).json({ error: { message: "Estado de Agencia inválido." } });
  const { rows } = await pool.query(
    ...
    [req.params.id,status,req.rumboSession.account_id],
  );
  if (!rows[0]) return res.status(404).json({ error: { message: "Agencia no encontrada." } });
  await audit(req.rumboSession.email, "retailer.status_changed", "retailer", req.params.id, { status });
  res.json(rows[0]);
});

bootstrapAdmin()
  .then(() => app.listen(PORT, "0.0.0.0", () => console.log(`Rumbo API listening on ${PORT}`)))
  .catch((error) => { console.error("Rumbo API bootstrap failed", error); process.exit(1); });
